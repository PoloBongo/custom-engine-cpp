...
static ImGui_ImplGlfw_Data* ImGui_ImplGlfw_GetBackendData()
{
	return ImGui::GetCurrentContext()
		       ? static_cast<ImGui_ImplGlfw_Data*>(ImGui::GetIO().BackendPlatformUserData)
		       : nullptr;
}
...
static bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api)
{
	...
	#ifdef _WIN32
	bd->GlfwWndProc = (WNDPROC)GetWindowLongPtrW(static_cast<HWND>(main_viewport->PlatformHandleRaw), GWLP_WNDPROC);
	IM_ASSERT(bd->GlfwWndProc != nullptr);
	SetWindowLongPtrW(static_cast<HWND>(main_viewport->PlatformHandleRaw), GWLP_WNDPROC,
...
void ImGui_ImplGlfw_Shutdown()
{
	...
	#ifdef _WIN32
	ImGuiViewport* main_viewport = ImGui::GetMainViewport();
	SetWindowLongPtrW(static_cast<HWND>(main_viewport->PlatformHandleRaw), GWLP_WNDPROC, (LONG_PTR)bd->GlfwWndProc);
...
static void ImGui_ImplGlfw_UpdateMouseData()
{
	...
	// (those braces are here to reduce diff with multi-viewports support in 'docking' branch)
	{
		...
		if (is_window_focused)
		{
			...
			if (io.WantSetMousePos) glfwSetCursorPos(window, io.MousePos.x, io.MousePos.y);
			...
			if (bd->MouseWindow == nullptr)
			{
				...
				bd->LastValidMousePos = ImVec2(static_cast<float>(mouse_x), static_cast<float>(mouse_y));
