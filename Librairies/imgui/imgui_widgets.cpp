...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
	...
	if (hovered)
	{
		...
		for (int button = 0; button < 3; button++)
		{
			...
			// Handle ImGuiButtonFlags_MouseButtonRight and ImGuiButtonFlags_MouseButtonMiddle here.
			{
				...
				if (IsMouseReleased(button, test_owner_id) && mouse_button_released == -
				    1)
			...
			if (flags & ImGuiButtonFlags_PressedOnRelease)
				if (mouse_button_released != -1)
				{
					const bool has_repeated_at_least_once =
						(item_flags & ImGuiItemFlags_ButtonRepeat) && g.IO.MouseDownDurationPrev[mouse_button_released]
	...
	if (g.NavId == id && !g.NavDisableHighlight && g.NavDisableMouseHover) if (!(
		flags & ImGuiButtonFlags_NoHoveredOnFocus)) hovered = true;
                        ...
                        ImS64         size_contents_v, ImDrawFlags flags)
{
	...
	const float grab_h_pixels = ImClamp(
		scrollbar_size_v * (static_cast<float>(size_avail_v) / static_cast<float>(win_size_v)), style.GrabMinSize,
		scrollbar_size_v);
	...
	if (held && allow_interaction && grab_h_norm < 1.0f)
	{
		...
		if (g.ActiveIdIsJustActivated)
		{
			...
			else g.ScrollbarClickDeltaToGrabCenter               = clicked_v_norm - grab_v_norm - grab_h_norm * 0.5f;
		}
	...
	if (overlay_size.x > 0.0f)
		RenderTextClipped(
			ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x,
			       ...
			       bb.Min.y), bb.Max, overlay, nullptr, &overlay_size, ImVec2(0.0f, 0.5f), &bb);
}
                             ...
                             float         min_size2, float hover_extend, float hover_visibility_delay, ImU32 bg_col)
{
	...
	if (!ItemAdd(bb, id, nullptr, ImGuiItemFlags_NoNav)) return false;
	...
	if (hovered) g.LastItemData.StatusFlags |= ImGuiItemStatusFlags_HoveredRect;
...
static int IMGUI_CDECL ShrinkWidthItemComparer(const void* lhs, const void* rhs)
{
	...
	if (int d = static_cast<int>(b->Width - a->Width)) return d;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	if (flags & ImGuiComboFlags_WidthFitPreview)
		IM_ASSERT(
		(flags & (ImGuiComboFlags_NoPreview | static_cast<ImGuiComboFlags>(ImGuiComboFlags_CustomPreview))) == 0);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
	...
	else
	{
		...
		if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize) == 0 || g.NextWindowData.SizeVal.x <= 0.0f)
			...
			constraint_min.x = w;
		if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize) == 0 || g.NextWindowData.SizeVal.y <=
		    0.0f)
                  ...
                  void*       user_data, int items_count, int            popup_max_height_in_items)
{
	...
	for (int i = 0; i < items_count; i++)
	{
		...
		if (item_text == nullptr) item_text = "*Unknown item*";
                                ...
                                const char* format)
{
	...
	if (data_type == ImGuiDataType_S32 || data_type == ImGuiDataType_U32)
		return ImFormatString(
			buf, buf_size, format, *static_cast<const ImU32*>(p_data));
	if (data_type == ImGuiDataType_S64 || data_type == ImGuiDataType_U64)
		return ImFormatString(
			buf, buf_size, format, *static_cast<const ImU64*>(p_data));
	if (data_type == ImGuiDataType_Float)
		return ImFormatString(buf, buf_size, format,
		                      *static_cast<const float*>(p_data));
	if (data_type == ImGuiDataType_Double)
		return ImFormatString(buf, buf_size, format,
		                      *static_cast<const double*>(p_data));
	if (data_type == ImGuiDataType_S8) return ImFormatString(buf, buf_size, format, *static_cast<const ImS8*>(p_data));
	if (data_type == ImGuiDataType_U8) return ImFormatString(buf, buf_size, format, *static_cast<const ImU8*>(p_data));
	if (data_type == ImGuiDataType_S16)
		return
			ImFormatString(buf, buf_size, format, *static_cast<const ImS16*>(p_data));
	if (data_type == ImGuiDataType_U16)
		return
			ImFormatString(buf, buf_size, format, *static_cast<const ImU16*>(p_data));
	IM_ASSERT(0);
	return 0;
}
		...
		case ImGuiDataType_S8:
			if (op == '+')
				*static_cast<ImS8*>(output) = ImAddClampOverflow(
					*static_cast<const ImS8*>(arg1), *static_cast<const ImS8*>(arg2), IM_S8_MIN, IM_S8_MAX);
			if (op == '-')
		...
		case ImGuiDataType_U8:
			if (op == '+')
				*static_cast<ImU8*>(output) = ImAddClampOverflow(
					*static_cast<const ImU8*>(arg1), *static_cast<const ImU8*>(arg2), IM_U8_MIN, IM_U8_MAX);
			if (op == '-')
		...
		case ImGuiDataType_Float:
			if (op == '+')
				*static_cast<float*>(output) =
					*static_cast<const float*>(arg1) + *static_cast<const float*>(arg2);
			if (op == '-')
				*static_cast<float*>(output) =
					*static_cast<const float*>(arg1) - *static_cast<const float*>(arg2);
			return;
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
	...
	else format = ImParseFormatSanitizeForScanning(format, format_sanitized, IM_ARRAYSIZE(format_sanitized));
	...
	if (type_info->Size < 4)
	{
		if (data_type == ImGuiDataType_S8)
			*static_cast<ImS8*>(p_data) = static_cast<ImS8>(ImClamp(
				v32, static_cast<int>(IM_S8_MIN), static_cast<int>(IM_S8_MAX)));
		else if (data_type == ImGuiDataType_U8)
			*static_cast<ImU8*>(p_data) = static_cast<ImU8>(ImClamp(
				v32, static_cast<int>(IM_U8_MIN), static_cast<int>(IM_U8_MAX)));
		else if (data_type == ImGuiDataType_S16)
			*static_cast<ImS16*>(p_data) = static_cast<ImS16>(ImClamp(
				v32, static_cast<int>(IM_S16_MIN), static_cast<int>(IM_S16_MAX)));
		else if (data_type == ImGuiDataType_U16)
			*static_cast<ImU16*>(p_data) = static_cast<ImU16>(ImClamp(
				v32, static_cast<int>(IM_U16_MIN), static_cast<int>(IM_U16_MAX)));
		else
			IM_ASSERT(0);
	}
                          ...
                          const char*   format, ImGuiSliderFlags flags)
{
	...
	if (v_cur == static_cast<TYPE>(-0)) v_cur = static_cast<TYPE>(0);
	...
	switch (data_type)
	{
		case ImGuiDataType_S8:
			{
				...
				bool  r   = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
				                                               ...
				                                               p_max ? *static_cast<const ImS8*>(p_max) : IM_S8_MAX,
				                                               format, flags);
		...
		case ImGuiDataType_U8:
			{
				...
				bool  r   = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
				                                               ...
				                                               p_max ? *static_cast<const ImU8*>(p_max) : IM_U8_MAX,
				                                               format, flags);
		...
		case ImGuiDataType_S64:
			return DragBehaviorT<ImS64, ImS64, double>(data_type, static_cast<ImS64*>(p_v), v_speed,
			                                           ...
			                                           p_max ? *static_cast<const ImS64*>(p_max) : IM_S64_MAX, format,
			                                           flags);
		case ImGuiDataType_U64:
			return DragBehaviorT<ImU64, ImS64, double>(data_type, static_cast<ImU64*>(p_v), v_speed,
			                                           ...
			                                           p_max ? *static_cast<const ImU64*>(p_max) : IM_U64_MAX, format,
			                                           flags);
		case ImGuiDataType_Float:
		...
		case ImGuiDataType_Double:
			return DragBehaviorT<double, double, double>(data_type, static_cast<double*>(p_v), v_speed,
			                                             ...
			                                             p_max ? *static_cast<const double*>(p_max) : DBL_MAX, format,
			                                             flags);
		case ImGuiDataType_COUNT:
			break;
	}
                       ...
                       const void* p_max, const char*   format, ImGuiSliderFlags flags)
{
	...
	if (temp_input_is_active)
	{
		...
		const bool is_clamp_input = (flags & ImGuiSliderFlags_AlwaysClamp) != 0 && (
			                            p_min == nullptr || p_max == nullptr || DataTypeCompare(data_type, p_min, p_max)
			                            < 0);
                                  ...
                                  float         logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
	...
	if (is_logarithmic)
	{
		...
		else if ((v_max == 0.0f) && (v_min < 0.0f)) v_max_fudged = -logarithmic_zero_epsilon;
		...
		{
			float zero_point_center = (-static_cast<float>(v_min)) / (
			...
			if (v == 0.0f) result = zero_point_center; // Special case for exactly zero
				...
				result = (1.0f - static_cast<float>(
					          ImLog(-static_cast<FLOATTYPE>(v_clamped) / logarithmic_zero_epsilon) / ImLog(
						          -v_min_fudged / logarithmic_zero_epsilon))) * zero_point_snap_L;
			else
				result = zero_point_snap_R + (static_cast<float>(
					                              ImLog(static_cast<FLOATTYPE>(v_clamped) / logarithmic_zero_epsilon) /
					                              ImLog(
                                 ...
                                 float         logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
	...
	if (is_logarithmic)
	{
		...
		if ((v_min * v_max) < 0.0f) // Range crosses zero, so we have to do this in two parts
		{
			...
			float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
			float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
			if (t_with_flip >= zero_point_snap_L && t_with_flip <= zero_point_snap_R) result = static_cast<TYPE>(0.0f);
	...
	else
	{
		...
		else if (t < 1.0)
		{
			...
			result                = static_cast<TYPE>(static_cast<SIGNEDTYPE>(v_min) + static_cast<SIGNEDTYPE>(
				                                          v_new_off_f + (FLOATTYPE)(v_min > v_max ? -0.5 : 0.5)));
		}
	}
                           ...
                           const void*   p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
	...
	switch (data_type)
	{
		...
		case ImGuiDataType_S32:
			IM_ASSERT(
				*static_cast<const ImS32*>(p_min) >= IM_S32_MIN / 2 && *static_cast<const ImS32*>(p_max) <= IM_S32_MAX /
				2);
		...
		case ImGuiDataType_S64:
			IM_ASSERT(
				*static_cast<const ImS64*>(p_min) >= IM_S64_MIN / 2 && *static_cast<const ImS64*>(p_max) <= IM_S64_MAX /
				2);
			return SliderBehaviorT<ImS64, ImS64, double>(bb, id, data_type, static_cast<ImS64*>(p_v),
			                                             *static_cast<const ImS64*>(p_min),
			                                             *static_cast<const ImS64*>(p_max), format, flags, out_grab_bb);
		case ImGuiDataType_U64:
			...
			return SliderBehaviorT<ImU64, ImS64, double>(bb, id, data_type, static_cast<ImU64*>(p_v),
			                                             ...
			                                             *static_cast<const ImU64*>(p_max), format, flags, out_grab_bb);
		case ImGuiDataType_Float:
			IM_ASSERT(
		...
		case ImGuiDataType_Double:
			IM_ASSERT(
				*static_cast<const double*>(p_min) >= -DBL_MAX / 2.0f && *static_cast<const double*>(p_max) <= DBL_MAX /
				2.0f);
...
const char* ImParseFormatFindEnd(const char* fmt)
{
	...
	constexpr unsigned int ignored_lowercase_mask =
                        ...
                        const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
	...
	flags |= ImGuiInputTextFlags_AutoSelectAll | static_cast<ImGuiInputTextFlags>(ImGuiInputTextFlags_NoMarkEdited);
...
{
	return InputTextEx(label, nullptr, buf, static_cast<int>(buf_size), size, flags | ImGuiInputTextFlags_Multiline,
	                   callback, user_data);
}
                                     ...
                                     const ImWchar** remaining, ImVec2*  out_offset, bool           stop_on_new_line)
{
	...
	if (out_offset) *out_offset = ImVec2(line_width, text_size.y + line_height);
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
	...
	if (BufTextLen != pos) memmove(Buf + pos + new_text_len, Buf + pos, static_cast<size_t>(BufTextLen - pos));
                                     ...
                                     ImGuiInputTextCallback callback, void*    user_data, ImGuiInputSource input_source)
{
	                                     ...
	                                     ImGuiInputTextFlags_CharsScientific)))
	{
		...
		if (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsScientific)) if (
			c == '.' || c == ',') c = c_decimal_point;
		...
		// Allow 0-9 a-F A-F
		if (flags & ImGuiInputTextFlags_CharsHexadecimal) if (
			!(c >= '0' && c <= '9') && !(c >= 'a' && c <= 'f') && !(c >= 'A' && c <= 'F')) return false;

		// Turn a-z into A-Z
		if (flags & ImGuiInputTextFlags_CharsUppercase) if (
			c >= 'a' && c <= 'z') c += static_cast<unsigned int>('A' - 'a');

		if (flags & ImGuiInputTextFlags_CharsNoBlank) if (ImCharIsBlankW(c)) return false;
                                                         ...
                                                         int                  new_length_a)
{
	...
	auto new_buf = static_cast<ImWchar*>(static_cast<void*>(g.TempBuffer.Data));
                        ...
                        ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	else
	{
		...
		ItemSize(total_bb, style.FramePadding.y);
		if (!(flags & ImGuiInputTextFlags_MergedItem)) if (!
			ItemAdd(total_bb, id, &frame_bb, ImGuiItemFlags_Inputable)) return false;
	}
	const bool hovered = ItemHoverable(frame_bb, id, g.LastItemData.InFlags);
	...
	if ((init_state && g.ActiveId != id) || init_changed_specs || init_reload_from_user_buf)
	{
		...
		if (flags & ImGuiInputTextFlags_AlwaysOverwrite) state->Stb.insert_mode = 1;
		...
		if (render_selection)
		{
			...
			for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
			{
				...
				else
				{
					...
					if (rect_size.x <= 0.0f) rect_size.x = IM_TRUNC(g.Font->GetCharAdvance(' ') * 0.50f);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (InputText("##Text", buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsUppercase))
		{
			...
			else r = sscanf(p, "%02X%02X%02X", (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2]);
		...
		if (new_H <= 0 && H > 0)
		{
			if (new_V <= 0 && V != new_V)
				ColorConvertHSVtoRGB(H, S, new_V <= 0 ? V * 0.5f : new_V, col[0], col[1],
				                     col[2]);
			else if (new_S <= 0) ColorConvertHSVtoRGB(H, new_S <= 0 ? S * 0.5f : new_S, new_V, col[0], col[1], col[2]);
		}
	}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
	...
	else
	{
		...
		else window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), rounding);
	}
	RenderNavHighlight(bb, id);
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
	{
		if (g.Style.FrameBorderSize > 0.0f) RenderFrameBorder(bb.Min, bb.Max, rounding);
		else window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), rounding);
	...
	if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
	{
		...
		else SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F, &col_rgb, sizeof(float) * 4, ImGuiCond_Once);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	if (!is_leaf)
	{
		if (pressed && g.DragDropHoldJustPressedId != id)
		{
			if ((flags & (ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_OpenOnDoubleClick)) == 0 || (
				    g.NavActivateId == id))
				toggled = true;
			if (flags & ImGuiTreeNodeFlags_OpenOnArrow) toggled |= is_mouse_x_over_arrow && !g.NavDisableMouseHover;
		...
		if (flags & ImGuiTreeNodeFlags_Bullet)
			RenderBullet(window->DrawList,
			             ImVec2(text_pos.x - text_offset_x * 0.60f,
			                    text_pos.y + g.FontSize * 0.5f), text_col);
		else if (!is_leaf)
	...
	else RenderText(text_pos, label, label_end, false);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (span_all_columns)
	{
		...
		else if (window->DC.CurrentColumns) PushColumnsBackground();
		g.LastItemData.StatusFlags |= ImGuiItemStatusFlags_HasClipRect;
		g.LastItemData.ClipRect = window->ClipRect;
	}
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
	...
	{
		auto p = static_cast<char*>((void*)ImTextFindPreviousUtf8Codepoint(
	...
	if (buffer_len == 0) return nullptr;
	if (select_request)
	{
	...
	if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode)
	{
		...
		{
			if (memcmp(buf_begin, p, static_cast<size_t>(c0_len)) != 0) break;
		}
                                 ...
                                 const char* (*get_item_name_func)(void*, int), void* user_data, int nav_item_idx)
{
	...
	if (req->SingleCharMode && (
		...
		idx = TypingSelectFindNextSingleCharMatch(
			req, items_count, get_item_name_func,
                                               ...
                                               int nav_item_idx)
{
	...
	for (int idx = 0; idx < items_count; idx++)
	{
		...
		if (ImStrimatchlen(req->SearchBuffer, req->SearchBuffer + req->SingleCharSize, item_name) < req->
		    SingleCharSize)
			continue;
                  ...
                  const ImVec2& size_arg)
{
	...
	if (values_count >= values_count_min)
	{
		...
		if (hovered && inner_bb.Contains(g.IO.MousePos))
		{
			...
			else if (plot_type == ImGuiPlotType_Histogram) SetTooltip("%d: %8.4g", v_idx, v0);
			idx_hovered = v_idx;
		}
...
static float Plot_ArrayGetter(void* data, int idx)
{
	...
	const float v         = *static_cast<const float*>(static_cast<const void*>(
                                 ...
                                 ImGuiWindowFlags window_flags)
{
	...
	if (bar_window == nullptr || bar_window->BeginCount == 0)
	{
		...
		else if (dir == ImGuiDir_Down || dir == ImGuiDir_Right) viewport->BuildWorkOffsetMax[axis] -= axis_size;
	}
...
void ImGui::EndMainMenuBar()
{
	...
	ImGuiContext& g = *GImGui;
	if (g.CurrentWindow == g.NavWindow && g.NavLayer == ImGuiNavLayer_Main && !g.
	    NavAnyRequest)
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
	...
	{
		if (menu_is_open) menu_is_open = BeginPopupEx(id, window_flags);
			// menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
		else g.NextWindowData.ClearFlags(); // we behave like Begin() and need to consume those values
		return menu_is_open;
	}
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
	...
	if ((flags & ImGuiTabBarFlags_Reorderable) != (tab_bar->Flags & ImGuiTabBarFlags_Reorderable) || (
		...
		ImQsort(
			tab_bar->Tabs.Data, tab_bar->Tabs.Size, sizeof(ImGuiTabItem), TabItemComparerByBeginOrder);
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
	...
	{
		if (TabBarProcessReorder(tab_bar)) if (tab_bar->ReorderRequestTabId == tab_bar->
		                                       SelectedTabId) scroll_to_tab_id = tab_bar->ReorderRequestTabId;
	...
	else width_excess = (section_0_w + section_2_w) - tab_bar->BarRect.GetWidth();
...
{
	if (order < 0 || order >= tab_bar->Tabs.Size) return nullptr;
	return &tab_bar->Tabs[order];
}

ImGuiTabItem* ImGui::TabBarGetCurrentTab(ImGuiTabBar* tab_bar)
{
	if (tab_bar->LastTabItemIdx <= 0 || tab_bar->LastTabItemIdx >= tab_bar->Tabs.Size) return nullptr;
                      ...
                      ImGuiWindow* docked_window)
{
	...
	if (flags & ImGuiTabItemFlags_NoCloseButton) p_open = nullptr;
	else if (p_open == nullptr) flags |= ImGuiTabItemFlags_NoCloseButton;
	...
	if (!is_tab_button)
	{
		if (tab_appearing && (tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs) && tab_bar->NextSelectedTabId ==
		    0) if (!tab_bar_appearing || tab_bar->SelectedTabId == 0) TabBarQueueFocus(tab_bar, tab);
	...
	if (!tab_contents_visible && tab_bar->SelectedTabId == 0 && tab_bar_appearing)
		if (tab_bar->Tabs.Size == 1 && !(tab_bar->Flags &
	...
	if (text_clipped && g.HoveredId == id && !held)
		if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(
			...
			SetItemTooltip(
				"%.*s", static_cast<int>(FindRenderedTextEnd(label) - label), label);
...
void ImGui::SetTabItemClosed(const char* label)
{
	...
	if (is_within_manual_tab_bar)
	{
		...
		if (ImGuiTabItem* tab = TabBarFindTabByID(tab_bar, tab_id)) tab->WantClose = true;
                                       ...
                                       bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
	...
	if (out_text_clipped) *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
