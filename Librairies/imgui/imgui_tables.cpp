                         ...
                         const ImVec2& outer_size, float inner_width)
{
	...
	PushOverrideID(id);
	if (instance_no > 0) PushOverrideID(instance_id);
	...
	if (inner_window != outer_window)
	{
		if (flags & ImGuiTableFlags_BordersOuterV)
			table->HostClipRect.Min.x = ImMin(
				table->HostClipRect.Min.x + TABLE_BORDER_SIZE,
				table->HostClipRect.Max.x);
		if (flags & ImGuiTableFlags_BordersOuterH)
			table->HostClipRect.Min.y = ImMin(
				table->HostClipRect.Min.y + TABLE_BORDER_SIZE,
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
	{
		...
		else table->LeftMostEnabledColumn = static_cast<ImGuiTableColumnIdx>(column_n);
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
	{
		...
		if (column->NextEnabledColumn == -1 && table->LeftMostStretchedColumn != -
		    1)
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
	{
		...
		column->NavLayerCurrent = static_cast<ImS8>(
		...
		if (is_hovering_table && mouse_skewed_x >= column->ClipRect.Min.x && mouse_skewed_x < column->ClipRect.Max.
		    x)
	...
	if (is_hovering_table && table->HoveredColumnBody == -1)
		if (mouse_skewed_x >= unused_x1)
			table->HoveredColumnBody = static_cast<ImGuiTableColumnIdx>(table->
				ColumnsCount);
	if (has_resizable == false && (table->Flags &
	...
	if (table->IsContextPopupOpen && table->ContextPopupColumn != -1 && table->InstanceInteracted == table->
		...
		if (g.ActiveId == 0 || (table->IsActiveIdInTable || g.
		                        DragDropActive))
	...
	else inner_window->DrawList->PushClipRect(inner_window->ClipRect.Min, inner_window->ClipRect.Max, false);
}
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
	...
	ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
	constexpr float hit_half_width = TABLE_RESIZE_SEPARATOR_HALF_THICKNESS;
                             ...
                             ImGuiID     user_id)
{
	...
	if (table->IsInitializing)
	{
		...
		{
			if ((flags & ImGuiTableColumnFlags_WidthFixed) && init_width_or_weight >
			    0.0f)
		...
		case ImGuiTableBgTarget_CellBg:
			{
				...
				if (table->RowCellDataCurrent < 0 || table->RowCellData[table->RowCellDataCurrent].Column !=
				    column_n)
...
void ImGui::TableEndRow(ImGuiTable* table)
{
	...
	if (is_visible)
	{
		...
		if (table->HoveredColumnBody != -1 && g.IO.MousePos.y >= bg_y1 && g.IO.MousePos.y <
		    bg_y2)
		...
		if (table->CurrentRow > 0 && (table->Flags &
		...
		if (top_border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y)
			window->DrawList->
			        AddLine(ImVec2(table->BorderX1, bg_y1), ImVec2(table->BorderX2, bg_y1), top_border_col,
			                border_size);

		// Draw bottom border at the row unfreezing mark (always strong)
		if (draw_strong_bottom_border && bg_y2 >= table->BgClipRect.Min.y && bg_y2 < table->BgClipRect.Max.y)
			window->
				DrawList->AddLine(ImVec2(table->BorderX1, bg_y2), ImVec2(table->BorderX2, bg_y2),
				                  table->BorderColorStrong,
				                  border_size);
	}
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
	...
	if (column->Flags & ImGuiTableColumnFlags_IndentEnable) start_x += table->RowIndentOffsetX;
...
void ImGui::TableEndCell(ImGuiTable* table)
{
	...
	float* p_max_pos_x;
	if (table->RowFlags & ImGuiTableRowFlags_Headers) p_max_pos_x = &column->ContentMaxXHeadersUsed;
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
	{
		...
		if (column->IsVisibleX && column->IsVisibleY)
		{
			...
			column->DrawChannelUnfrozen = static_cast<ImGuiTableDrawChannelIdx>(
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
	...
	{
		merge_groups[n].ChannelsMask = static_cast<ImBitArrayPtr>(static_cast<void*>(
			g.TempBuffer.Data + (size_for_masks_bitarrays_one * n)));
	}
	auto remaining_mask = static_cast<ImBitArrayPtr>(static_cast<void*>(
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
	{
		...
		for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
		{
			...
			if (src_channel->_CmdBuffer.Size > 0 && src_channel->_CmdBuffer.back().ElemCount == 0 && src_channel->
			    _CmdBuffer.back().UserCallback == nullptr) // Equivalent of PopUnusedDrawCmd()
			...
			{
				float content_max_x;
				if (!has_freeze_v) content_max_x = ImMax(column->ContentMaxXUnfrozen, column->ContentMaxXHeadersUsed);
					// No row freeze
				else if (merge_group_sub_n == 0)
					content_max_x = ImMax(column->ContentMaxXFrozen,
					                      column->ContentMaxXHeadersUsed);
					// Row freeze: use width before freeze
				else content_max_x = column->ContentMaxXUnfrozen; // Row freeze: use width after freeze
		...
		for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
		{
			if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
			{
				...
				if ((merge_group_n & 1) == 0 || !has_freeze_h)
					merge_clip_rect.Min.x = ImMin(
						merge_clip_rect.Min.x, host_rect.Min.x);
				if ((merge_group_n & 2) == 0 || !has_freeze_v)
					merge_clip_rect.Min.y = ImMin(
						merge_clip_rect.Min.y, host_rect.Min.y);
				if ((merge_group_n & 1) != 0) merge_clip_rect.Max.x = ImMax(merge_clip_rect.Max.x, host_rect.Max.x);
				if ((merge_group_n & 2) != 0 && (table->Flags & ImGuiTableFlags_NoHostExtendY) ==
				    0)
...
ImGuiSortDirection ImGui::TableGetColumnNextSortDirection(ImGuiTableColumn* column)
{
	...
	for (int n = 0; n < 3; n++)
	{
		if (column->SortDirection == TableGetColumnAvailSortDirection(column, n))
			return
				TableGetColumnAvailSortDirection(column, (n + 1) % column->SortDirectionsAvailCount);
	}
	IM_ASSERT(0);
...
void ImGui::TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs)
{
	...
	if (column->SortDirection == ImGuiSortDirection_None) column->SortOrder = -1;
		...
		column->SortOrder = append_to_sort_specs
			                    ? sort_order_max + 1
			                    : 0;
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
	{
		if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
			if ((table->Columns[column_n].Flags & ImGuiTableColumnFlags_NoHeaderLabel) == 0)
				row_height = ImMax(
					row_height, CalcTextSize(TableGetColumnName(table, column_n)).y);
	}
...
void ImGui::TableHeadersRow()
{
	...
	if (IsMouseReleased(1) && TableGetHoveredColumn() == columns_count) if (
		mouse_pos.y >= row_y1 && mouse_pos.y < row_y1 + row_height) TableOpenContextMenu(columns_count);
...
void ImGui::TableHeader(const char* label)
{
	...
	else
	{
		...
		if ((table->RowFlags & ImGuiTableRowFlags_Headers) == 0)
			TableSetBgColor(
				ImGuiTableBgTarget_CellBg, GetColorU32(ImGuiCol_TableHeaderBg), table->CurrentColumn);
	}
	RenderNavHighlight(bb, id, ImGuiNavHighlightFlags_Compact | ImGuiNavHighlightFlags_NoRounding);
	if (held) table->HeldHeaderColumn = static_cast<ImGuiTableColumnIdx>(column_n);
	...
	if (text_clipped && hovered && g.ActiveId == 0) SetItemTooltip("%.*s", static_cast<int>(label_end - label), label);
		...
		for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
		{
			...
			if (pass == 0)
			{
				...
				while (label_name < label_name_end)
				{
					...
					if (label_name_eol == nullptr) label_name_eol = label_name_end;
					...
					// Using padding.y*2.0f would be symetrical but hide more text.
					...
					auto pivot_in  = ImVec2(window->ClipRect.Min.x, window->ClipRect.Min.y + label_size.y);
					...
					column->ContentMaxXHeadersUsed      =
						column->ContentMaxXHeadersIdeal = column->WorkMinX + ImCeil(line_off_curr_x);
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display)
{
	...
	if (flags_for_section_to_display & ImGuiTableFlags_Resizable)
	{
		...
		else size_all_desc = LocalizeGetMsg(ImGuiLocKey_TableSizeAllDefault); // "###SizeAll" All stretch or mixed
	...
	if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
	{
		...
		for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
		{
			...
			if (MenuItem(name, nullptr, other_column->IsUserEnabled, menu_item_active))
				other_column->
					IsUserEnabledNextFrame = !other_column->IsUserEnabled;
		}
		PopItemFlag();
	}
}
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
	...
	for (int data_n = 0; data_n < settings->ColumnsCount; data_n++, column_settings++)
	{
		...
		else column->DisplayOrder = static_cast<ImGuiTableColumnIdx>(column_n);
...
void ImGui::TableGcCompactSettings()
{
	...
	for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != nullptr;
	     settings                     = g.SettingsTables.next_chunk(settings))
	{
		if (settings->ID != 0) required_memory += static_cast<int>(TableSettingsCalcChunkSize(settings->ColumnsCount));
	}
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
	...
	if ((columns->Flags & ImGuiOldColumnFlags_NoPreserveWidths))
		x = ImMin(
			x, ImGui::GetColumnOffset(column_index + 1) - g.
...
static float GetColumnWidthEx(ImGuiOldColumns* columns, int column_index, bool before_resize = false)
{
	...
	else offset_norm = columns->Columns[column_index + 1].OffsetNorm - columns->Columns[column_index].OffsetNorm;
...
void ImGui::EndColumns()
{
	...
	if (!(flags & ImGuiOldColumnFlags_GrowParentContentsSize)) window->DC.CursorMaxPos.x = columns->HostCursorMaxPosX;
	...
	if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
	{
		...
		for (int n = 1; n < columns->Count; n++)
		{
			...
			constexpr float     column_hit_hw = COLUMNS_HIT_RECT_HALF_WIDTH;
			const ImRect        column_hit_rect(ImVec2(x - column_hit_hw, y1), ImVec2(x + column_hit_hw, y2));
			if (!ItemAdd(column_hit_rect, column_id, nullptr, ImGuiItemFlags_NoNav)) continue;
