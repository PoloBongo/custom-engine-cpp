		...
		for (bool exit = false; !exit;)
		{
			...
			for (auto&& message : messages)
			{
				if (message->is<Bousk::Network::Messages::Connection>())
				{
					...
					{
						std::scoped_lock lock(coutMutex);
						std::cout << "Connexion inattendue de " << message->emitter().toString() << " (devait être de " << client2.toString() << ")" << std::endl;
						continue;
					}
					else
					{
						std::scoped_lock lock(coutMutex);
						std::cout << "Client 2 [" << client2.toString() << "] connecté à client 1" << std::endl;
					}
				}
				else if (message->is<Bousk::Network::Messages::UserData>())
				{
					...
					{
						std::cout << "Erreur de désérialisation du message !" << std::endl;
						return;
					}
					...
					{
						std::scoped_lock lock(coutMutex);
						std::cout << "Tous les messages reçus en ordre !" << std::endl;
						std::cout << "Déconnexion du client 2..." << std::endl;
						exit = true;
					}
				}
				...
				for (auto&& message : messages)
				{
					if (message->is<Bousk::Network::Messages::IncomingConnection>())
					{
						if (message->emitter() != client1) {
							std::scoped_lock lock(coutMutex);
							std::cout << "Connexion entrante inattendue de " << message->emitter().toString() << " (devrait être " << client1.toString() << ")" << std::endl;
						...
						{
							std::scoped_lock lock(coutMutex);
							std::cout << "Client 2 reçoit une demande de connexion de [" << message->emitter().toString() << "] (client 1)... et l’accepte" << std::endl;
						}
					...
					else if (message->is<Bousk::Network::Messages::Connection>())
					{
						...
						{
							std::scoped_lock lock(coutMutex);
							std::cout << "Connexion inattendue de " << message->emitter().toString() << " (devrait être " << client1.toString() << ")" << std::endl;
							continue;
						}
						else
						{
							std::scoped_lock lock(coutMutex);
							std::cout << "Client 1 [" << client1.toString() << "] connecté à client 2" << std::endl;
						}
						// Envoi des chaînes de caractères à client 1, 1 chaîne par message
						for (const auto& msg : messagesToSend)
						{
							...
							{
								std::scoped_lock lock(coutMutex);
								std::cout << "Erreur de sérialisation de la chaîne \"" << msg << "\" !" << std::endl;
								return;
							}
					...
					{
						// Attente que client 1 se déconnecte
						if (message->is<Bousk::Network::Messages::Disconnection>())
						{
							std::scoped_lock lock(coutMutex);
							assert(message->emitter() == client1);
							std::cout << "Déconnexion de client 1... [" << static_cast<int>(message->as<Bousk::Network::Messages::Disconnection>()->reason) << "]" << std::endl;
							exit = true;
						}
					}
				}
