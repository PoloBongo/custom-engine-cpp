#include "UDP/Protocol/Simulator.h"
	...
	namespace Network
	{
		namespace UDP
		{
			...
			void Simulator::push(const Datagram& datagram, const Address& from)
			{
				...
				const bool shouldDuplicate = std::uniform_int_distribution(1, static_cast<int>(mDuplicateRate.Max()))(mDuplicateGenerator) < mDuplicate.get();
